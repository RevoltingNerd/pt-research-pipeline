...
def build_governance_sheet(l2_df: pd.DataFrame) -> pd.DataFrame:
    """Governance flags extracted from all L2 articles — key for the SIG chair."""
    if l2_df.empty:
        return pd.DataFrame()
    ...
    # Re-read governance fields from JSON (they're not all in the L2 sheet above)
    deep_dir = Path("summaries/deep")
    gov_data = {}
    for f in deep_dir.glob("*_layer2.json"):
        ...
        pmid = str(d.get("pmid", ""))
        gov_data[pmid] = {
            "Governance Recommendations":      d.get("governance_recommendations", ""),
            "Governance Claim Without Method": d.get("governance_claim_without_method", ""),
            "Governance Gap Detail":           d.get("governance_gap_detail", ""),
    ...
    for col in ["Governance Recommendations", "Governance Claim Without Method",
                "Governance Gap Detail", "Patient Safety Concerns",
                "Ethical Considerations", "Implementation Barriers",
                "Future Research", "Governance Synthesis",
                "Internal Consistency", "Limitations Stated"]:
        df[col] = df["PMID"].map(lambda p: gov_data.get(p, {}).get(col, ""))

    # Flag rows with active governance findings
    df["Has Governance Recs"] = df["Governance Recommendations"].apply(
        lambda x: "YES" if x and x.lower() not in ("none stated", "", "not_stated") else "no"
    )
    df["Governance Gap"] = df["Governance Claim Without Method"].apply(
        lambda x: "⚠️ YES" if x.lower() == "yes" else "no"
    )

    # Sort: governance gaps first, then those with recs, then rest
    df["_sort"] = df.apply(
        lambda r: 0 if r["Governance Gap"] == "⚠️ YES"
                  else 1 if r["Has Governance Recs"] == "YES"
                  else 2, axis=1
    )
    df = df.sort_values(["_sort", "Cluster"]).drop(columns=["_sort"])
    return df
