...
LAYER0_OPTIONS = {}  # populated from config.yaml (model.layer0_options) in main()
LAYER0_CHAR_LIMIT = 8000  # populated from config.yaml (pipeline.layer0_char_limit) in main()
...
def call_ollama(prompt: str, base_url: str) -> dict:
    # Defaults match the original phi4:14b 8K-staged config. Override via
    # model.layer0_options in config.yaml (e.g. for qwen3.6 full-text mode:
    # num_ctx: 40000, think: false).
    opts = LAYER0_OPTIONS
    payload = {
        "model":  LAYER0_MODEL,
        "prompt": prompt,
        ...
        "options": {
            ...
            "num_predict":  opts.get("num_predict", 800),
...
def run_layer0_on_article(article: dict, cfg: dict) -> dict:
    pmid      = article["pmid"]
    full_text_raw = article.get("full_text_content", "")
    if LAYER0_CHAR_LIMIT == "full":
        full_text = full_text_raw
    else:
        full_text = full_text_raw[:LAYER0_CHAR_LIMIT]
...
def main(limit=None, pmids=None, process_all=False):
    global LAYER0_MODEL, LAYER0_OPTIONS, LAYER0_CHAR_LIMIT
    ...
    LAYER0_OPTIONS = cfg["model"].get("layer0_options", {})
    LAYER0_CHAR_LIMIT = cfg.get("pipeline", {}).get("layer0_char_limit", 8000)
    ...
    if pmids:
        ...
        # Layer 0 screens everything ingest produced — appraisal_complete is
        # set by later layers and is not a valid pre-filter here.
        targets = df
